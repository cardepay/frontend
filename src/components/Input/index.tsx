...
const InputIcon = ({ type,icon, id, placeholder,errorMsg}) => {
  const [error, triggerError]= useState(false);
  ...
  return (
    ...
    <input type={type} id={id} placeholder={placeholder} onChange={InputHandler} className={`ring-none outline-none p-3 text-xs transition-all ${error ? 'ring-red' : 'ring-black/25 focus:ring-black/50'}`}/>
      </div>
   <p className={`text-red transition-all select-none text-xs ${!error && 'hidden'}`}>{ errorMsg || 'Incorrect '  }</p>
</div>
  )
